...
if ( ! function_exists( 'uncode_post_navigation' ) ) :
	...
	function uncode_post_navigation($index_btn = '', $nextprev_title = 'off', $navigation_index = '', $generic = true) {
		...
		$prev_label = ($nextprev_title === 'on') ? (isset($previous->post_title) ? $previous->post_title : '') : esc_html__('ANTERIOR','uncode');
		$next_label = ($nextprev_title === 'on') ? (isset($next->post_title) ? $next->post_title : '') : esc_html__('SIGUIENTE','uncode');
