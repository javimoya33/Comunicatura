						...
						foreach ($team_socials as $key => $value) {
							...
							if ($value !== '') {
								if(filter_var($value, FILTER_VALIDATE_EMAIL)) {
									$inner_entry .= '<a href="mailto:'.$value.'">'.$value.'</a>';
								} else {
	...
	function uncode_get_info_box($out, $atts) {
		...
		switch ($out) {
			...
			case 'author':
				...
				$output .= '<span class="author-info">Por <a href="'.get_author_posts_url( $author_id ).'">'.get_the_author_meta( 'display_name' , $author_id ).'</a></span>';
				$output .= '</span>';
				return $output;
				break;
